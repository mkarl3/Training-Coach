...
// ── Hero trajectory: CTL / ATL / TSB lines, provisional tail faded, trend zones shaded,
//    tripwire events marked. Hand-drawn SVG so provisional/zone rendering is fully controlled.
function Trajectory({ state }) {
  const data = state.trajectory;
  const W = 880, H = 300, pad = { l: 34, r: 12, t: 12, b: 22 };
  const valid = data.filter((d) => d.ctl != null);
  if (valid.length < 2) return <div className="empty">Not enough data in window.</div>;
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="fitness trajectory">
        {/* trend warning zones (shaded bands) */}
        ...
        {/* provisional (soft trailing edge) */}
        {provX != null && (
      ...
      <div className="legend">
        ...
        <span><i style={{ background: "var(--prov)" }} />provisional (recomputing)</span>
      </div>
    </div>
  );
}
...
function WatchPanel({ state }) {
  ...
  return (
    <div className="panel">
      <h2>Watch out for</h2>
      {nothing && <div className="empty">✓ Nothing active — no failure-mode signal right now.</div>}
      {tripwires.map((t, n) => (
...
function Gauge({ gauge }) {
  return (
    <div className="panel">
      ...
      {!gauge && <div className="empty">No long-ride durability data yet.</div>}
      {gauge && (
...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${API}/api/meta`).then((r) => r.json()).then((mt) => {
      setMeta(mt);
      const ds = daysBetween(mt.date_min, mt.date_max);
      setDates(ds);
      const start = ds.indexOf("2026-03-25");          // open on the crash so the view has signal
      setIdx(start >= 0 ? start : ds.length - 1);
  ...
  useEffect(() => {
    if (!asOf) return;
    fetch(`${API}/api/watchman?as_of=${asOf}&window=120`)
      .then((r) => r.json()).then(setState).catch((e) => setErr(String(e)));
  }, [asOf]);

  if (err) return <div className="wrap"><h1>Watchman</h1><p style={{ color: "var(--red)" }}>API error: {err}. Is the backend on :8000?</p></div>;
  if (!meta || !state) return <div className="wrap"><h1>Watchman</h1><p>Loading…</p></div>;

  return (
    <div className="wrap">
      <header>
        <h1>Watchman <small>where you're headed, and what to watch for</small></h1>
